...
import { NameListProps, Person, detectCollisions } from "../models";
...
const NameList: React.FC<NameListProps> = ({ people, showAvatar = true, showTaskCount = true, theme }) => {
    ...
    return (
        <div
            className="name-list w-64 flex-shrink-0 bg-white border-r border-gray-200 z-10"
            data-testid="gantt-name-list">
            {/* Header */}
            ...
            <div className="people-list">
                {people.map((person: Person, index: number) => {
                    // Calculate row height based on task collisions
                    const collisionRows = detectCollisions(person.tasks);
                    const rowHeight = Math.max(60, collisionRows.length * 40);
